...
// Returns a PinnedStagingPool when AOTI_COPY_USE_PINNED_ASYNC is set to a
// non-zero / non-empty value AND host pinning succeeds. Returns nullptr
// otherwise so callers fall back to the synchronous copy path. Per-buffer
// size comes from AOTI_COPY_STAGE_BUFFER_BYTES (default 64 MiB). getenv is
// cached for process lifetime.
inline std::unique_ptr<PinnedStagingPool> tryMakeConstantsStagingPool() {
  static const char* enable_env = std::getenv("AOTI_COPY_USE_PINNED_ASYNC");
  const bool enabled = enable_env != nullptr && enable_env[0] != '\0' &&
      enable_env[0] != '0';
  if (!enabled) {
    return nullptr;
  }
  static const char* size_env = std::getenv("AOTI_COPY_STAGE_BUFFER_BYTES");
  size_t buf_bytes = PinnedStagingPool::kDefaultBufferBytes;
  if (size_env != nullptr && size_env[0] != '\0') {
    try {
      auto parsed = std::stoull(size_env);
      if (parsed > 0) {
        buf_bytes = static_cast<size_t>(parsed);
      }
    } catch (...) {
      // Ignore parse errors; use default.
    }
  }
  return PinnedStagingPool::tryCreate(buf_bytes);
}
...
class AOTInductorModelBase {
 public:
  ...
  void load_constants(bool force = false) {
    ...
    PinnedStagingPool* _pool_raw = _staging_pool.get();
#endif
    ...
    // loaded constants are observed by callers.
    _staging_pool.reset();
#endif
